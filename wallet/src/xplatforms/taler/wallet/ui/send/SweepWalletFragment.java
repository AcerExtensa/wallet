...
import xplatforms.taler.wallet.Configuration;
import xplatforms.taler.wallet.Constants;
import xplatforms.taler.wallet.R;
import xplatforms.taler.wallet.WalletApplication;
import xplatforms.taler.wallet.data.DynamicFeeLoader;
import xplatforms.taler.wallet.data.PaymentIntent;
import xplatforms.taler.wallet.ui.AbstractBindServiceActivity;
import xplatforms.taler.wallet.ui.DialogBuilder;
import xplatforms.taler.wallet.ui.InputParser.StringInputParser;
import xplatforms.taler.wallet.ui.ProgressDialogFragment;
import xplatforms.taler.wallet.ui.ScanActivity;
import xplatforms.taler.wallet.ui.TransactionsAdapter;
import xplatforms.taler.wallet.util.MonetarySpannable;
import xplatforms.taler.wallet.util.WalletUtils;
...
public class SweepWalletFragment extends Fragment
{
    ...
    private ECKey new_privateKey = null;
    ...
    public void onActivityResult(final int requestCode, final int resultCode, final Intent intent)
    {
        if (requestCode == REQUEST_CODE_SCAN)
        {
            if (resultCode == Activity.RESULT_OK)
            {
                ...
                //Log.d("EXDBG:", "onActivityResult  "+ input);
                ...
                new StringInputParser(input)
                {
                    ...
                    {
                        //Log.d("EXDBG:", "handlePrivateKey in SweepWalletFragment");

                        //Log.d("XDBG:", "privkey: "+key.toString());
                        //Log.d("XDBG:", "privkey2: "+key.toBase58());
                        //Log.d("XDBG:", "privkey3: "+key.getVersion());
                        //Log.d("XDBG:", "privkey4: "+key.hashCode());
                        //;
                    ...
                    protected void handlePaymentIntent(final PaymentIntent paymentIntent)
                    {
                        ...
                        //Log.d("EXDBG:", "SweepWalletFragment handlePaymentIntent !!!!!!");
    ...
    private void handleReload()
    {
        ...
        //Log.d("EXDBG:", "HANDLE Reload???? request WalletBalance!!!!!");

        //http://explorer.taler.site/ext/getbalance/+wallet
...
//        Log.d("EXDBG:", "ECKey: " + key.toString());
//        Log.d("EXDBG:", "ECKey: " + key.getPrivateKeyAsWiF(Constants.NETWORK_PARAMETERS));
//        Log.d("EXDBG:", "ECKey: " + key.getPublicKeyAsHex());
//        Log.d("EXDBG:", "ECKey: " + key.toAddress(Constants.NETWORK_PARAMETERS).getHash160());
//        Log.d("EXDBG:", "ECKey: " + key.toAddress(Constants.NETWORK_PARAMETERS).toBase58());
//        Log.d("EXDBG:", "ECKey: " + key.toAddress(Constants.NETWORK_PARAMETERS).toString());
//        Log.d("EXDBG:", "ECKey: " + key.isCompressed());
    ...
    {
        //Log.d("EXDBG:", "requestWalletBalance!!!");
        ...
        {
            @Override
            public void onResult(final Coin balance)
            {
                ProgressDialogFragment.dismissProgress(fragmentManager);

                Transaction ftx;
                ftx = new Transaction(Constants.NETWORK_PARAMETERS);
                ftx.getConfidence().setConfidenceType(TransactionConfidence.ConfidenceType.BUILDING);
                final Address address = walletToSweep.getImportedKeys().iterator().next().toAddress(Constants.NETWORK_PARAMETERS);
                //Log.d("EXDBG:", "This should be private key address: "+ address.toString());
                final Script script = ScriptBuilder.createOutputScript(address);
                final TransactionOutput fout = new TransactionOutput(Constants.NETWORK_PARAMETERS, ftx, balance, script.getProgram());
                ftx.addOutput(fout);

                walletToSweep.clearTransactions(0);
                walletToSweep.addWalletTransaction(new WalletTransaction(WalletTransaction.Pool.UNSPENT, ftx));
        ...
        new RequestWalletBalanceTask(backgroundHandler, callback).requestWalletBalance(new_privateKey.getPrivateKeyAsWiF(Constants.NETWORK_PARAMETERS), address);
    }
    ...
    private void handleSweep()
    {
        ...
        Wallet wa = application.getWallet();
        wa.importKey(this.new_privateKey);
