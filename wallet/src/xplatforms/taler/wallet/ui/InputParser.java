...
import xplatforms.taler.wallet.util.Io;
import xplatforms.taler.wallet.util.Qr;
...
public abstract class InputParser {
    ...
    public abstract static class StringInputParser extends InputParser {
        ...
        public void parse()
        {
            ...
            Log.d("DBGQR", "InputParser ->parse -> input " +input);

            if (input.startsWith("TALER:-"))
            {
                try
                {
            ...
            else if (input.startsWith("taler:"))
            {
                try
                {
                    ...
                    //Log.d("DBGQR", "input.startsWith(\"taler:\") " + address.toString());

                    //final Address address = Address.fromBase58(null, input);
                    ...
                    //Log.d("EXDBG:" , PATTERN_BITCOIN_ADDRESS.toString());
                    //Log.d("EXDBG:" , "PATERN MATCH " + PATTERN_BITCOIN_ADDRESS.matcher(address.toString()).matches());

                    //getResources().getString(R.string.hello_world);


                    handlePaymentIntent(PaymentIntent.fromAddress(address.toString(), null));
            ...
            else if (PATTERN_DUMPED_PRIVATE_KEY_UNCOMPRESSED.matcher(input).matches() || PATTERN_DUMPED_PRIVATE_KEY_COMPRESSED.matcher(input).matches())
            {
                try
                {
            ...
            else
            {
                ...
                try
                {
                    ...
                    NetworkParameters params = TalerCoinMain.get();
                        ...
                        {
                            //Log.d("EXDBG:", "DumpedPrivateKey.fromBase58(params, input);");
                            DumpedPrivateKey dumpedPrivateKey = DumpedPrivateKey.fromBase58(params, input);
                            key = dumpedPrivateKey.getKey();

                            //final VersionedChecksummedBytes kk = BIP38PrivateKey.fromBase58(params, key.toAddress(params).toString());


                            handlePrivateKey(dumpedPrivateKey);

                            //final Address address = new Address(Constants.NETWORK_PARAMETERS, dumpedPrivateKey.getKey().getPubKeyHash());
                            //handlePaymentIntent(PaymentIntent.fromAddress(address, null));

                        }
                        else
                        {
                            //Log.d("EXDBG:", "ECKey.fromPrivate(privKey);");
                    ...
                    final Address adr = new Address(Constants.NETWORK_PARAMETERS, key.getPubKeyHash());

                    //Log.d("EXDBG:", "final Address adr: " + adr.toString() + " handlePaymentIntent ->");

                    //handlePaymentIntent(PaymentIntent.fromAddress(adr, "Paper Wallet"));

                    //final VersionedChecksummedBytes vkey = new VersionedChecksummedBytes(key.)
                    ...
                    Log.d("TRYDBG:","TESTING INPUT PAPER WALLET");
                    TalerPrivateKeys.fromQrInput(input, "TEU5bnWLXNzUmLtSs9AWsWjCqPYvRk6M44");

                    final VersionedChecksummedBytes key = BIP38PrivateKey.fromBase58(Constants.NETWORK_PARAMETERS, input);
                    Log.d("EXDBG:", "VersionedChecksummedBytes "+key.toString());
                    handlePrivateKey(key);
                    */
